...
import { Search, Plus, RefreshCw, Package, CheckCircle2, AlertCircle, Clock, MoreHorizontal, Loader2, Trash2, X } from 'lucide-react';
import { useProducts, useProductSyncStatus, useDeleteProduct, useCreateProduct, useVendors } from '../../hooks/useApi';
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={createProduct.isPending}
              ...
              {createProduct.isPending ? <><Loader2 size={14} className="animate-spin" /> Adding…</> : 'Add Product'}
            </button>
          </div>
        </form>
      </motion.div>
                ...
                <motion.tr key={p._id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button
                        onClick={() => { if (window.confirm('Delete this product?')) deleteProduct.mutate(p._id); }}
